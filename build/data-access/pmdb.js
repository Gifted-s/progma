...
class PmDB {
  ...
  async getProjects() {
    ...
    let format = await db.collection('projects').aggregate([// First Stage
    {
      $group: {
        _id: "$project",
        users: {
  ...
  async insertProject(project) {
    ...
    if (exist.length > 0) {
      return {
        status: 'failed',
        message: 'User have already submitted this project'
      };
    }
  ...
  async insertProjectName(name) {
    const db = await this.createConnection();
    let name_exists = await db.collection('project_names').find({
  ...
  async updateProjectName(id, nameUpdateBody) {
    const db = await this.createConnection();
    let name_exists = await db.collection('project_names').find({
      project_name: nameUpdateBody.project_name
    });
    const name_array = await name_exists.toArray();
    ...
    let result = await db.collection('project_names').updateOne({
      "_id": _mongodb.default.ObjectID(id)
    }, {
      $set: { ...nameUpdateBody
      }
  ...
  async deleteProjectName(id) {
    const db = await this.createConnection();
    let result = await db.collection('project_names').deleteOne({
      "_id": _mongodb.default.ObjectID(id)
    });
